...
class FeedSystem {
  constructor(user) {
    this.user = user;
    this.sessionContext = user.sessionContext || { bucket: "A" }; // Default to A
  ...
  async fetchCandidatePosts() {
    ...
    try {
      ...
      // 1.5 Pushed Feed Items (Fan-out)
      // These are "Guaranteed" to be relevant as they were explicitly pushed
      const pushedSnapshot = await Firestore.firestore()
        .collection("users")
        .doc(this.user.id)
      ...
      if (!pushedSnapshot.empty) {
        const postIds = pushedSnapshot.docs.map((doc) => doc.data().postId);
        // Batch fetch the actual posts
        const pushedPosts = await Firestore.getByIds("posts", postIds);
        pushedPosts.forEach((post) => {
          if (post) allPosts.set(post.id, post);
        });
      }
  ...
  async fetchCandidateGames() {
    ...
    try {
      ...
      gamesSnapshot.forEach((doc) => {
        ...
        const matchesSport =
          interestedSports.includes(gameSport) ||
          favoriteSports.includes(gameSport);
        const matchesTags =
          userTags.length > 0 && gameTags.some((tag) => userTags.includes(tag));
  ...
  calculateBaseEngagement(item) {
    ...
    // AB Testing: Bucket B weights "Shares" higher
    if (this.sessionContext.bucket === "B") {
      const shareBonus = (item.shares?.length || 0) * 2;
      return score + shareBonus;
    }
  ...
  getColdStartBoost(author) {
    ...
    const accountAge =
      Date.now() - (author.createdAt?.toMillis?.() || Date.now());
  ...
  calculatePersonalizationScore(item, author) {
    ...
    // Tag matching
    const matchingTags = itemTags.filter(
      (tag) =>
  ...
  filterByQuality(items) {
    return items.filter((item) => {
      ...
      if (viewCount < QUALITY_THRESHOLDS.minViewCount) {
        const postAge =
          Date.now() - (item.createdAt?.toMillis?.() || Date.now());
  ...
  async fetchSuggestedUsers() {
    try {
      const following = this.user.following || [];
      const excludedIds = new Set([this.user.id, ...following]);
      const candidatesMap = new Map(); // userId -> { user: Object, score: Number }

      // ─────────────────────────────────────────────────────────────
      // 1. DATA COLLECTION (CANDIDATE SOURCES)
      // ─────────────────────────────────────────────────────────────

      // Source A: SOCIAL GRAPH (Friends of Friends)
      const graphCandidates = await this.getGraphCandidates(
        following,
        excludedIds,
      );
      this.mergeCandidates(candidatesMap, graphCandidates, "social");

      // Source B: INTEREST MATCHING (Tags & Sports)
      const interestCandidates = await this.getInterestCandidates(excludedIds);
      this.mergeCandidates(candidatesMap, interestCandidates, "interest");

      // Source C: COMPUTED SIMILARITY (Collaborative Filtering)
      const similarityCandidates =
        await this.getSimilarityCandidates(excludedIds);
      this.mergeCandidates(candidatesMap, similarityCandidates, "similarity");

      // ─────────────────────────────────────────────────────────────
      // 2. SCORING & RANKING
      // ─────────────────────────────────────────────────────────────
      const scoredCandidates = Array.from(candidatesMap.values()).map(
        ({ user, sources }) => {
          let score = 0;

          // Social score (mutual following)
          if (sources.social) score += sources.social * 10;

          // Interest score (overlap)
          score += this.calculateInterestOverlap(user) * 5;

          // Location bonus
          if (user.location === this.user.location) score += 5;
          if (user.country === this.user.country) score += 2;

          // Trust factor (reuse existing logic)
      ...
      // ─────────────────────────────────────────────────────────────
      let topSuggestions = scoredCandidates.slice(0, 30);

      if (topSuggestions.length < 10) {
        const fallbacks = await this.getPopularFallbacks(excludedIds, 20);
        topSuggestions = [...topSuggestions, ...fallbacks].slice(0, 30);
      }

      return this.categorizeUsersByRole(topSuggestions);
    } catch (error) {
  ...
  async getGraphCandidates(following, excludedIds) {
    const candidates = new Map();
    if (following.length === 0) return {};

    try {
      // Limit to first 20 people I follow to avoid massive queries
      const seeds = following.slice(0, 20);
      const seedUsers = await Firestore.getByIds("users", seeds);

      for (const seed of seedUsers) {
        if (!seed || !seed.following) continue;
      ...
      const hydrated = await Firestore.getByIds("users", topIds);
      const result = {};
      hydrated.forEach((u) => {
        if (u) result[u.id] = { user: u, weight: candidates.get(u.id) };
      });
      return result;
    } catch (e) {
      console.error("getGraphCandidates error:", e);
      return {};
    }
  }

  async getInterestCandidates(excludedIds) {
    const userTags = this.user.tags || [];
    const favoriteSports = this.user.favoriteSports || [];
    const candidates = new Map();

    try {
      // Query users with same tags (limit to few search keys)
      ...
      // Query users with same favorite sports if still low
      if (candidates.size < 20 && favoriteSports.length > 0) {
        const snapshot = await Firestore.firestore()
          .collection("users")
          .where(
            "favoriteSports",
            "array-contains-any",
            favoriteSports.slice(0, 10),
          )
          .limit(20)
          .get();
      ...
      candidates.forEach((u) => {
        result[u.id] = { user: u, weight: 1 };
      });
      return result;
    } catch (e) {
      console.error("getInterestCandidates error:", e);
      return {};
    }
  }

  async getSimilarityCandidates(excludedIds) {
    try {
      const similarityDoc = await Firestore.firestore()
        .collection("users")
        .doc(this.user.id)
        .collection("computed")
        .doc("similarity")
      ...
      if (!similarityDoc.exists) return {};

      const { similarUsers = [] } = similarityDoc.data();
      const filtered = similarUsers.filter((u) => !excludedIds.has(u.userId));
      const ids = filtered.map((u) => u.userId);

      const users = await Firestore.getByIds("users", ids);
      const result = {};
      users.forEach((u) => {
        if (u) {
          const score = filtered.find((f) => f.userId === u.id)?.score || 0;
          result[u.id] = { user: u, weight: score };
        }
      });
      return result;
    } catch (e) {
      console.error("getSimilarityCandidates error:", e);
      return {};
    }
  }

  mergeCandidates(map, newCandidates, sourceKey) {
    for (const id in newCandidates) {
      if (!map.has(id)) {
        map.set(id, { user: newCandidates[id].user, sources: {} });
      }
      map.get(id).sources[sourceKey] = newCandidates[id].weight;
    }
  }

  calculateInterestOverlap(other) {
    const userTags = new Set(this.user.tags || []);
    const otherTags = other.tags || [];
    const tagOverlap = otherTags.filter((t) => userTags.has(t)).length;

    const userSports = new Set(this.user.favoriteSports || []);
    const otherSports = other.favoriteSports || [];
    const sportOverlap = otherSports.filter((s) => userSports.has(s)).length;

    return tagOverlap + sportOverlap * 2;
  }

  async getPopularFallbacks(excludedIds, limit) {
    const roles = ["scout", "coach", "athlete", "team"];
    const results = [];
    ...
    for (const role of roles) {
      const snapshot = await Firestore.firestore()
        .collection("users")
        .where("role", "==", role)
        .limit(Math.ceil(limit / roles.length))
        .get();
  ...
  categorizeUsersByRole(users) {
    ...
    users.forEach((user) => {
      const role = (user.role || "fan").toLowerCase();
      // Map to plural keys
      const key =
        role === "scout"
          ? "scouts"
